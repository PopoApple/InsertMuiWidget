...
(function($, window, document) {
	...
	window.addEventListener($.START_EV, function(event) {
 ...
 */
(function($, window) {
	$.EVENT_START = $.START_EV;
	$.EVENT_MOVE = $.MOVE_EV;
	$.EVENT_END = $.END_EV;
	$.EVENT_CANCEL = $.CANCEL_EV;
	$.EVENT_CLICK = 'click';
	$.gestures = {
		session: {}
	};
	...
	var getTouches = function(event, touch) {
		var allTouches = event.touches?$.slice.call(event.touches || event):[event];
		...
		if ((type === $.EVENT_START || type === $.EVENT_MOVE) && allTouches.length === 1) {
			...
			var targetTouches = [];
			var changedTargetTouches = [];
			var changedTouches = event.changedTouches?$.slice.call(event.changedTouches || event):[event];
...
(function($, window) {
	...
	$.menu = function() {
		...
		if (menu) {
			$.trigger(menu, $.START_EV); //临时处理menu无touchstart的话，找不到当前targets的问题
			$.trigger(menu, 'tap');
		} else { //执行父窗口的menu
			if (window.plus) {
...
(function($, window, document, undefined) {
	...
	var Scroll = $.Class.extend({
		...
		_initEvent: function(detach) {
			...
			this.wrapper[action]($.START_EV, this);
			this.wrapper[action]($.CANCEL_EV, this);
			this.wrapper[action]($.END_EV, this);
		...
		_end: function(e) {
			...
			if ((!this.moved && this.needReset) || e.type === $.CANCEL_EV) {
				this.resetPosition();
			}
		},
	...
	var OffCanvas = $.Class.extend({
		...
		initEvent: function() {
			...
			if (this.classList.contains('mui-draggable')) {
				this.wrapper.addEventListener($.START_EV, this); //临时处理
...
(function($, window, document, name) {
	...
	var onPopoverShown = function(e) {
		...
		this.addEventListener($.MOVE_EV, $.preventDefault);
		$.trigger(this, 'shown', this);
	}
	var onPopoverHidden = function(e) {
		setStyle(this, 'none');
		this.removeEventListener('webkitTransitionEnd', onPopoverHidden);
		this.removeEventListener($.MOVE_EV, $.preventDefault);
	...
	var backdrop = (function() {
		...
		element.addEventListener($.MOVE_EV, $.preventDefault);
...
(function($, window, name) {
	...
	Toggle.prototype.initEvent = function() {
		this.element.addEventListener($.START_EV, this);
		this.element.addEventListener('drag', this);
		this.element.addEventListener('swiperight', this);
		this.element.addEventListener($.END_EV, this);
		this.element.addEventListener($.CANCEL_EV, this);
...
(function($, window, document) {
	...
	window.addEventListener($.MOVE_EV, function(event) {
		toggleActive(false);
	});
	...
	window.addEventListener($.END_EV, function(event) { //使用touchend来取消高亮，避免一次点击既不触发tap，doubletap，longtap的事件
		if (!cell) {
			return;
		}
		toggleActive(false);
	...
	window.addEventListener($.CANCEL_EV, function(event) { //使用touchcancel来取消高亮，避免一次点击既不触发tap，doubletap，longtap的事件
		if (!cell) {
			return;
		}
		toggleActive(false);
...
(function($, window, document) {
	...
	Input.prototype.initElementEvent = function() {
		...
		if (this.sliderActionClass) {
			...
			element.addEventListener($.MOVE_EV, function(e) {
				e.stopPropagation();
			});
		} else {
